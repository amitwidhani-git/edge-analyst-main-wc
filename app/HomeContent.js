...
export default function HomeContent({ initialData }) {
  ...
  const eloFromMatches = matches.reduce((acc,m) => {
    if (m.home && m.home_elo != null) acc[m.home] = m.home_elo;
    if (m.away && m.away_elo != null) acc[m.away] = m.away_elo;
    return acc;
  }, {});
  const getTeamElo = team => tmMap[team]?.elo ?? eloFromMatches[team] ?? 1500;
    ...
    <section data-theme="dark" className="ea-section-pad" style={{background:'#080808',padding:'0 56px 80px',borderTop:'1px solid rgba(247,245,240,.05)'}}>
      ...
      <div className="ea-groups-grid" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:2}}>
        {groups.map(g=>{
          ...
          const ranked=ts.map(t=>({team:t,elo:getTeamElo(t)})).sort((a,b)=>b.elo-a.elo);
